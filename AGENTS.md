...
- No `<script>` tags except the template's own `<script data-fss="form-status">` and `<script type="application/ld+json">` business data. No inline event handlers (`onclick=` etc.), no `javascript:` links, no iframes except a Google Maps embed (`https://www.google.com/maps/embed?...`), no external stylesheets except Google Fonts, no tracking pixels.
